...
###案例分析
>  如上，在MainActivity在新建的时候，初始占用内存1776。
...
> 只定义TestModel成员变量的情况下，内存占用1780=初始内存+引用类型（4）。
...
> 内存占用1788=case1+类信息（8），说明调用new时，即使是空对象，也需要8字节左右的堆空间存储关于描述该对象的类信息。
...
> 内存占用未变化，还是初始值1776，说明局部变量生命周期只存在于方法内部，方法结束时，即可被gc回收。
...
> 内存占用1777=初始状态+1，基础类型boolean的引用类型占用1字节。
...
> 内存占用1780=初始状态+4，装箱类型Boolean本质上是一个对象，由case1可以推导出引用类型占用4字节。
...
![case9_map.png](https://raw.githubusercontent.com/listen2code/article/master/性能优化（一）堆内存分析/screenshot/case9_map.png)
...
写法1：匿名类，内存占用=MainActivity（1780）+MainActivity$1（12）=1792。
```
public class MainActivity extends AppCompatActivity {
...
写法2：成员变量类，内存占用=MainActivity（1780）+MainActivity$1（12）+成员变量（4）=1796。
...
写法3：接口回调，内存占用=1780（减少成员变量，及new创建新的对象，内存占用最少）。
...
> 
* "aaaaa"这个String的内存占用26=类信息（8）+count（4）+hashCode（4）+char[]数组（10）。
...
> 一个空的StringBuilder就占49字节，而value这个字符数组占了32字节，而我们最多也就添加"aaaaac"6个字符，所以这里可以通过new StringBuilder(6)初始化字符数组的大小，避免浪费。
...
> 首先在StringBuilder初始化的时候设置了字符数组大小为6，所以StringBuilder的初始内存占用就变少了，而在完成append("aaaaa")，append("c")之后，只要当前字符数组的容量够用，就不会继续扩容，避免了String拼接时，多占用一个临时变量，浪费内存的问题。当然前提是控制好StringBuilder的char[]初始容量，不然扩容后也会造成一定的内存浪费。

###总结
> 
1.谨慎创建成员变量：不管有用没用，非基础类型的成员变量只要定义了，至少需要4字节，优先使用局部变量，缩短变量生命周期，促使GC更快回收。
